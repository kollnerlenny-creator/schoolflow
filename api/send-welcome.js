...
export default async function handler(req, res) {
    ...
    const { email, name, isOberstufe } = req.body;
    ...
    const stufenText = isOberstufe
        ? 'Viel Erfolg in der Oberstufe! Das Abitur packst du. 💪'
        : 'Viel Erfolg in der Schule! Du schaffst das. 💪';

    const stufenLabel = isOberstufe ? 'Gymnasiale Oberstufe' : 'Sekundarstufe I';
  ...
  <div style="max-width:520px;margin:40px auto;padding:0 16px;">
    ...
    <div style="background:linear-gradient(135deg,#4f46e5 0%,#7c3aed 100%);border-radius:24px;padding:36px 32px;text-align:center;margin-bottom:16px;position:relative;overflow:hidden;">
      ...
      <div style="position:relative;z-index:1;">
        ...
        <p style="margin:6px 0 0;color:rgba(255,255,255,0.65);font-size:13px;font-weight:600;letter-spacing:2px;text-transform:uppercase;">${stufenLabel}</p>
      </div>
    </div>
    ...
    try {
        await transporter.sendMail({
            from: `"SchoolFlow" <${process.env.GMAIL_USER}>`,
            to: email,
